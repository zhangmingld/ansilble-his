...
// of the "pod" on the given "nodeInfo".Node(). It returns four values: 1) whether
// targetPod matches affnityTerms and their topologies based on the request of affinity or anti-affinity 2) whether targetPod
...
func (c *PodAffinityChecker) podMatchesPodAffinityTerms(pod, targetPod *v1.Pod, nodeInfo *schedulernodeinfo.NodeInfo, checkAffinity bool) (bool, bool, error) {
	...
	if checkAffinity && !termsSelectorMatched {
		return false, false, err
	}
	...
	for _, term := range matchedTerms {
		...
		if topologyMatched {
			...
			} else if checkAffinity && matchedTermCount == affinityTermsNum && termsSelectorMatched {
